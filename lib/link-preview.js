...
function withoutStyleAndScript(html) {
  return String(html || "")
...
function hasHtmlClass(html, className) {
  ...
  return new RegExp(`\\bclass=["'][^"']*\\b${escaped}\\b`, "i").test(html);
}

function syndicationArtifactType(html) {
  const source = withoutStyleAndScript(html);
  if (
    hasHtmlClass(source, "syndication-artifact--pdf")
    || hasHtmlClass(source, "artifact--pdf")
    || hasHtmlClass(source, "synd-tree-link--pdf")
    || hasHtmlClass(source, "artifact-pdf-iframe")
  ) return "pdf";
  if (hasHtmlClass(source, "syndication-artifact--handout") || hasHtmlClass(source, "synd-tree-link--handout")) {
    return "handout";
  }
  return "";
}
...
function artifactType(url, html = "", stored = "") {
  ...
  const visible = withoutStyleAndScript(html);
  const hay = `${url}\n${visible}`.toLowerCase();
  if (/\.pdf(\?|#|"|'|\s|$)/.test(hay) || hay.includes("application/pdf") || /\bopen pdf\b/.test(hay)) {
    return "pdf";
  }
  for (const part of String(visible || "").split(/\n/)) {
...
function pdfNeedsRescrape(item, adventureTitle = "") {
  const raw = item && typeof item === "object" ? item : {};
  if ((Number(raw.linkPreviewVersion) || 0) < LINK_PREVIEW_VERSION) return true;
...
function fromHtml(url, html, fetchedAt = new Date().toISOString(), options = {}) {
  ...
  if (isSyndicationUrl(url)) {
    ...
    if (synd) {
      return { ...synd, artifactType: artifactType(url, clipped), fetchedAt, fetchError: "", linkPreviewVersion: LINK_PREVIEW_VERSION };
    }
  }
  const og = fromOpenGraph(url, clipped, options);
  return { ...og, artifactType: artifactType(url, clipped), fetchedAt, fetchError: "", linkPreviewVersion: LINK_PREVIEW_VERSION };
}
...
function failedPreview(url, message, fetchedAt = new Date().toISOString()) {
  ...
  return {
    ...
    linkPreviewVersion: LINK_PREVIEW_VERSION
  };
}
