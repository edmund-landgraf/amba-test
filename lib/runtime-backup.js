...
function coerceTime(item) {
  ...
  if (Boolean(time.liveOverride)) next.liveOverride = true;
  else delete next.liveOverride;
  if (Boolean(time.liveBlocked)) next.liveBlocked = true;
  else delete next.liveBlocked;
  return next;
}
...
function coerceReadingLink(item) {
  ...
  return {
    ...
    ...(Number(raw.linkPreviewVersion) > 0 ? { linkPreviewVersion: Number(raw.linkPreviewVersion) } : {})
  };
}
