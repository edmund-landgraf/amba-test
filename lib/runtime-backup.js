...
function coerceVoteNotes(notes, votes) {
  const out = {};
  for (const [key, value] of Object.entries(asObject(notes))) {
    if (votes[key] !== "maybe" && votes[key] !== "no") continue;
...
function coerceSignup(item, timeIds, usedHandles) {
  ...
  return {
    ...
    votes,
    voteNotes: coerceVoteNotes(signup.voteNotes, votes),
...
async function loadRuntime(rootDir) {
  ...
  return { site, users, adventures, feedback, wgExportIndex };
}
