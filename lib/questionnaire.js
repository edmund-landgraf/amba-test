...
function coerceQuestion(item, usedIds = new Set(), now = new Date().toISOString()) {
  ...
  const gm = coerceGmPreference(type, raw.gmPersonalChoice, raw.gmPreference, options);
  return {
    id,
    type,
    label,
    required: Boolean(raw.required),
    gmPersonalChoice: gm.gmPersonalChoice,
    gmPreference: gm.gmPreference,
    options,
...
function coerceGmPreference(type, enabled, value, options) {
  if (type === "select") {
    return { gmPersonalChoice: false, gmPreference: "" };
  }
  const gmPersonalChoice = Boolean(enabled);
  if (type === "checkbox") {
    const selected = asArray(value).map(cleanOption).filter(Boolean);
    const allowed = new Set(options);
    return {
      gmPersonalChoice,
      gmPreference: selected.filter((item, index) => allowed.has(item) && selected.indexOf(item) === index)
    };
  }
  ...
  if (type === "radio") {
    return {
      gmPersonalChoice,
      gmPreference: options.includes(text) ? text : ""
    };
  }
  return { gmPersonalChoice, gmPreference: text };
}

function gmPreferenceLabel(question) {
  const raw = asObject(question);
  if (raw.type === "select" || !raw.gmPersonalChoice) return "";
  if (raw.type === "checkbox") {
    const values = asArray(raw.gmPreference).map(cleanOption).filter(Boolean);
    return values.length ? `GM preference: ${values.join(", ")}` : "";
  }
  const text = cleanOption(raw.gmPreference);
