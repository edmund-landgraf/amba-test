...
const DEFAULT_DISCORD_HOSTS = [
  ...
  },
  {
    ...
    bannerHalfUrl: "/images/hosted-by-amba-half.png"
  }
];
...
function asHost(item) {
  ...
  return {
    ...
    bannerHalfUrl: sanitizeBannerUrl(raw.bannerHalfUrl) || null
  };
}

function hostedByBannerHeight(value) {
  return String(value || "").toLowerCase() === "half" ? "half" : "full";
}

function pickHostBannerUrl(host, height) {
  if (!host) return "";
  const full = sanitizeBannerUrl(host.bannerUrl);
  const half = sanitizeBannerUrl(host.bannerHalfUrl);
  if (hostedByBannerHeight(height) === "half") return half || full;
  return full || half;
}
...
function resolveDiscordHost(data, records) {
  ...
  if (name === MANUAL_HOST_VALUE) {
    ...
    return {
      ...
      bannerHalfUrl: sanitizeBannerUrl(data.bannerHalfUrl) || null
    };
  }
  ...
  return {
    ...
    bannerHalfUrl: sanitizeBannerUrl(data.bannerHalfUrl) || preset.bannerHalfUrl || null
  };
}
